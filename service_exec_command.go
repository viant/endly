...
type ExtractableCommandRequest struct {
	...
	Target             *url.Resource       `required:"true" description:"host where command runs" example:"{\"URL\":\"ssh://127.0.0.1/\", \"Credential\":\"${env.HOME}/.secret/localhost.json\"}" `           //execution target - destination where to run a command.
...
type CommandRequest struct {
	SuperUser bool          `description:"flag to run as super user, in this case sudo will be added to all individual commands unless present, and Target.Credentials password will be used"` ///flag to run it as super user
	Target    *url.Resource `required:"true" description:"host where command runs" example:"{\"URL\":\"ssh://127.0.0.1/\", \"Credential\":\"${env.HOME}/.secret/localhost.json\"}" `           //execution target - destination where to run a command.
	Commands  []string      `required:"true" description:"command list" example:"[\"ls -al\", \"cp src dsc\"]"`                                                                                //list of commands to run
