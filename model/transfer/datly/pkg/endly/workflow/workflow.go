...
type WorkflowInput struct {
	...
	ID        []string          `parameter:",kind=form,in=id" predicate:"in,group=0,w,ID"`
...
type WorkflowView struct {
	...
	Task          []*TaskView  `on:"Id:ID=WorkflowId:WORKFLOW_ID" sql:"uri=workflow/task.sql"`
	Asset         []*AssetView `on:"Id:ID=WorkflowId:WORKFLOW_ID" sql:"uri=workflow/asset.sql"`
}
...
type TaskView struct {
	...
	Async         *int    `sqlx:"ASYNC"`
	SkipExpr      *string `sqlx:"SKIP_EXPR"`
	IsTemplate    *int    `sqlx:"IS_TEMPLATE"`
